...
Task = namedtuple('Task', ['category', 'action', 'order'])
...
named_tasks = [Task._make(el) for el in tasks]

for el, group in groupby(sorted(named_tasks, key=lambda x: (x.category, x.action, x.order)), key=lambda x: x.category):
    print(f'{el}:')
    for x in sorted(group, key= lambda x: x.order):
        print(f'''    {x.order}. {x.action}''')
    print()
